...
if($db) {
	...
	switch ($range_type) {
		...
		$opens_query = "SELECT sm_mailing_summary.Mailing_Nm, sm_tracking_log.TType, sm_tracking_log.MailingId, date_format(Datestamp, '%Y-%m-%d') as Datestamp, COUNT(sm_tracking_log.TType) AS OpenCount
						FROM `sm_tracking_log`
						INNER JOIN sm_mailing_summary
						ON sm_tracking_log.MailingID = sm_mailing_summary.Mailing_Id
						WHERE Datestamp BETWEEN '$start_no_time' AND '$end_no_time'  AND sm_mailing_summary.Mailing_Nm LIKE '$specific_email' AND TType ='open'
		...
		$opens_query = "SELECT sm_mailing_summary.Mailing_Nm, sm_tracking_log.TType, sm_tracking_log.MailingId, date_format(Datestamp, '%Y-%m-%d') as Datestamp, COUNT(sm_tracking_log.TType) AS OpenCount
						...
						INNER JOIN sm_mailing_summary
						ON sm_tracking_log.MailingID = sm_mailing_summary.Mailing_Id
						WHERE Datestamp LIKE '".$date_no_time."%' AND sm_mailing_summary.Mailing_Nm LIKE '$specific_email' AND TType ='open'
						GROUP BY sm_tracking_log.MailingID, date_format(Datestamp, '%Y-%m-%d')";
		...
		$opens_query = "SELECT sm_mailing_summary.Mailing_Nm, sm_tracking_log.TType, sm_tracking_log.MailingId, date_format(Datestamp, '%Y-%m-%d') as Datestamp, COUNT(sm_tracking_log.TType) AS OpenCount
						...
						INNER JOIN sm_mailing_summary
						ON sm_tracking_log.MailingID = sm_mailing_summary.Mailing_Id
						WHERE Datestamp BETWEEN '$date_no_time' AND '$date_no_time' + INTERVAL 7 DAY AND sm_mailing_summary.Mailing_Nm LIKE '$specific_email' AND TType ='open'
						GROUP BY sm_tracking_log.MailingID, date_format(Datestamp, '%Y-%m-%d')";
		...
		$opens_query = "SELECT sm_mailing_summary.Mailing_Nm, sm_tracking_log.TType, sm_tracking_log.MailingId, date_format(Datestamp, '%Y-%m-%d') as Datestamp, COUNT(sm_tracking_log.TType) AS OpenCount
						...
						INNER JOIN sm_mailing_summary
						ON sm_tracking_log.MailingID = sm_mailing_summary.Mailing_Id
						WHERE Datestamp BETWEEN '$date_no_time' AND '$date_no_time' + INTERVAL 30 DAY AND sm_mailing_summary.Mailing_Nm LIKE '$specific_email' AND TType ='open'
						GROUP BY sm_tracking_log.MailingID, date_format(Datestamp, '%Y-%m-%d')";
